...
def next_event(Events):
    """Cette fonction gère l'enchaînement des évènements."""
    try:
        #On récupère le prochain évènement
        next_event = Events.pop(0) 
        
        if  isinstance(next_event, Product_arrival):
            """Ici on gère l'évènement de type 'arrivée produit'.'"""
            #On récupère le numéro du produit
            num_prod = next_event.number
            product = Products[num_prod - 1]
            #On détermine la 1ere machine où doit passer le produit
            num_machine = product.path[0]
            machine = Machines[num_machine -1]
            #On regarde si la liste est vide
            if not machine.queue == 0 : 
                machine.queue.append(product)
                product.arrivals.append(next_event.time)
                print(str(next_event.time)+' : '+colored('Product'+
                    str(num_prod), 'green')+' arrives at'+colored(' Machine '+
                    str(num_machine), 'red')
                    )
                #On lance l'affectation d'employés
        ...
        if isinstance(next_event, Liberation):
            """Ici on gère l'évènement de type 'libération machine'.'"""
            ...
            machine.occupation = 0
            print (
                str(next_event.time)+' : '+colored('Machine '+
                str(machine_num), 'red')+' is liberated by'+
                colored(' Worker ' + str(next_event.worker_num), 'cyan' )
                )
            #on pourra songer à indiquer dans occupied le worker qui travaille sur la machine 0 (vide , 1 ,2,3,4  ), idem pour les machines
            #On regarde si le produit doit passer par d'autres machines
            if (next_event.product.path.index(machine_num) +1 !=
                len(next_event.product.path)):
                next_machine_number = next_event.product.path[
                    next_event.product.path.index(machine_num)+1]
                Machines[next_machine_number -1].queue.append(
                    next_event.product)
                print(
                    str(next_event.time)+' : '+colored('Product '+
                    str(next_event.product.number), 'green')+' is sent to'+
                    colored(' Machine '+ str(next_machine_number),'red')
                    )
            
            else : 
                print(str(next_event.time)+' : '+colored('Product '+
                    str(next_event.product.number),'green')+' is finished'
                    ) 
                product = Products[next_event.product.number-1]
                product.departures.append(next_event.time)

            #On ajoute le travailleur à la file de travailleurs libres
            Waiting_workers.append(Workers[next_event.worker_num - 1])        
